 ...
 *   часы × ставка + доля от стоимости проведённой ВТ
 *   + премия за чек-лист (все шесть пунктов)
 *   + премия за ЕГИСЗ.
 *   Встреча 22.09: фокус смещён с визуальной терапии на обязательную работу
 *   помощника — доля ВТ снижена 30% → 20%, чек-лист поднят 3 000 → 10 000,
 *   ЕГИСЗ 2 000 → 5 000 (Юлия Геннадьевна: «чек-лист должен быть тысяч
 *   десять, чтобы он мотивировал»). Ориентир собственника: 45–50 тыс.
 */
(function (global) {
  ...
  var DEFAULT_NORMS = {
    ...
    помощник: {
      ...
      чекЛист: { порог: 90, премия: 10000 },       // % выполненных пунктов ≥ порога → премия
                                                  // (на 6 пунктах 90% = все шесть, 83% = 5 из 6)
      егисз: { премия: 5000 },
      долиВыручки: { визуальнаяТерапия: 0.20 }  // стартовое значение поля «доля от ВТ»,
                                              // руководитель крутит его прямо в калькуляторе
    }
  };
